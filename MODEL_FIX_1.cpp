	...
	double PROBABLY)
{
	...
	double epsM = vectorSPH[vectorSPH.size()-1][0];

	while (eps1 + 2 * eps2 < epsM)
	{
		if (eps1 + 2 * eps2 > vectorSPH[j][0])
		{
			std::vector<double> vectorLine;
			//vectorLine.push_back(eps1);
			//vectorLine.push_back(eps2);
			vectorLine.push_back(-SIG1 / 1e9);
			vectorLine.push_back(-SIG2 / 1e9);
			vectorLine.push_back(VOLP / VOLS);
			vectorModel.push_back(vectorLine);
			vectorLine.clear();
			//std::cout << std::endl << "  EPS=" << eps1 + eps2 * 2 << "  SIG11=" << -SIG1 / 1e9 << "   SIG22=" << -SIG2 / 1e9 << 
			// "   POROSITY=" << VOLP / VOLS << "  Tay11=" << TAU1 << "   Wpl=" << Wpl << "  RhoD=" << RHOD << std::endl;
			
			PROBABLY = 0.01e0 * (pow(pow
			(abs((vectorModel[j][0] - vectorSPH[j][5]) / vectorDelta[5]), 2)+ 
				abs((vectorModel[j][1] - vectorSPH[j][6]) / vectorDelta[6]), 2)+ 
				pow(abs((vectorModel[j][2] - vectorSPH[j][9]) / vectorDelta[9]), 2));
			if (PROBABLY < 100.00)
			{
